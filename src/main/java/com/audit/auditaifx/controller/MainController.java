...
public class MainController {
    ...
    private String role = "ADMIN"; // Par défaut ADMIN pour ne pas casser l'existant
    ...
    public void initialize() {
        ...
        colStatut.setCellValueFactory(new PropertyValueFactory<>("scoreAudit"));
        ...
        // On ne force plus le dashboard ici, on attendra setRole ou l'appel explicite
        // showDashboard(); 
    }
        ...
        filteredList = service.getTous().stream()
            .filter(r -> {
                ...
                boolean matchStatut = statut == null || statut.equals("Tous")
                    || (r.getScoreAudit() != null && r.getScoreAudit().contains(statut)); // Recherche simplifiée par score
    ...
    @FXML
    public void showReportDetails(RapportAudit rapport) {
        ...
        // Rafraîchir risques depuis la BD (toujours recharger depuis DB pour avoir les derniers)
        List<Risque> risquesDB = service.getRisques(rapport.getId());
        if (risquesDB != null && !risquesDB.isEmpty()) {
            rapport.setRisques(risquesDB);
        }
        rafraichirTableauRisques(rapport);
        
        // Message si aucun risque
        if (rapport.getRisques().isEmpty()) {
            risqueTable.setPlaceholder(new Label(" Aucun risque détecté pour ce rapport."));
        } else {
            risqueTable.setPlaceholder(new Label("Chargement des risques..."));
        }
    ...
    private void ouvrirFormulaireRapport(RapportAudit rapport) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/rapports_reco/rapport-form-view.fxml"));
    ...
    private void ouvrirFormulaireReco(Recommandation reco) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/rapports_reco/reco-form-view.fxml"));
            Parent root = loader.load();
    ...
    private void afficherBadgeScore(String scoreStr) {
        try {
            double score;
            // Support both JSON format and plain number format (ex: '7,8')
            if (scoreStr != null && scoreStr.trim().startsWith("{")) {
                com.google.gson.JsonObject data = com.google.gson.JsonParser.parseString(scoreStr).getAsJsonObject();
                score = data.get("global").getAsDouble();
            } else {
                score = Double.parseDouble(scoreStr.replace(",", "."));
            }
            
            lblScoreBadge.setText("Score Rapport: " + String.format("%.1f", score).replace(".", ",") + "/10");
            lblScoreBadge.getStyleClass().removeAll("score-badge-green", "score-badge-orange", "score-badge-red");
            
            if (score >= 7.5) lblScoreBadge.getStyleClass().add("score-badge-green");
            else if (score >= 5.0) lblScoreBadge.getStyleClass().add("score-badge-orange");
            ...
            System.err.println("Erreur badge score: " + e.getMessage());
            // Show raw value if parsing fails
            if (lblScoreBadge != null) {
                lblScoreBadge.setText("Score Rapport: " + scoreStr + "/10");
                lblScoreBadge.setVisible(true);
                lblScoreBadge.setManaged(true);
            }
        }
    ...
    private void ouvrirModalScore(String json) {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("/fxml/rapports_reco/score-modal.fxml"));
    ...
    public void genererRisquesIA() {
        ...
        // Vérifier si le score est bon, ne pas générer de risques
        try {
            double score = 0;
            String scoreStr = rapportSelectionne.getScoreAudit();
            if (scoreStr != null && scoreStr.trim().startsWith("{")) {
                com.google.gson.JsonObject data = com.google.gson.JsonParser.parseString(scoreStr).getAsJsonObject();
                score = data.get("global").getAsDouble();
            ...
            if (score >= 8.0) {
                Alert info = new Alert(Alert.AlertType.INFORMATION);
                info.setTitle("Analyse des Risques");
                info.setHeaderText("Bilan de Sécurité");
                info.setContentText("Aucun risque sur cette entreprise. Le score (" + scoreStr + "/10) indique une conformité élevée.");
                info.show();
                
                // Mettre à jour le message placeholder
                risqueTable.setPlaceholder(new Label(" Aucun risque sur cette entreprise."));
                return;
            }
        } catch (Exception ex) {
            // Ignorer l'erreur de parsing et générer un risque par précaution
        }
    ...
    public void switchClient() {
        try {
            var resource = getClass().getResource("/fxml/rapports_reco/client-view.fxml");
            if (resource == null) {
