...
public class RapportService {
    ...
    public void modifier(RapportAudit r) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql);
            ...
            // Synchroniser les recommandations (Nettoyage simple pour démo)
            st.execute("DELETE FROM audit_reco WHERE rapport_id='" + r.getId() + "'");
            for (Recommandation reco : r.getRecommandations()) {
                ajouterRecommandation(r.getId(), reco);
            }
            
            // Synchroniser les risques
            st.execute("DELETE FROM audit_risque WHERE rapport_id='" + r.getId() + "'");
            for (Risque risque : r.getRisques()) {
                ajouterRisque(r.getId(), risque);
            }
