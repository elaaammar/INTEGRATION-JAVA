...
public class RegisterController {
    ...
    private void handleRegister() {
        ...
        if (!com.example.mindjavafx.util.Validation.isValidName(nom)) {
            errorLabel.setText("❌ Nom invalide");
            return;
        }

        if (!com.example.mindjavafx.util.Validation.isValidEmail(email)) {
        ...
        if (!com.example.mindjavafx.util.Validation.isValidAge(age)) {
            errorLabel.setText("❌ Âge doit être entre 18 et 100");
            return;
        }

        if (!telephone.isEmpty() && !com.example.mindjavafx.util.Validation.isValidTelephone(telephone)) {
